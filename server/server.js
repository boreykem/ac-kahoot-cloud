...
// Seed Super Admin in MongoDB — and sync password from userData.json
const seedSuperAdmin = async () => {
  try {
    // Load the real password from userData.json if available
    let realAdminPassword = 'admin123';
    try {
      const USERS_FILE_PATH = path.join(__dirname, 'userData.json');
      if (fs.existsSync(USERS_FILE_PATH)) {
        const parsed = JSON.parse(fs.readFileSync(USERS_FILE_PATH, 'utf-8'));
        const adminEntry = parsed.find(u => u.role === 'superadmin' || u.email === 'baureykem@gmail.com');
        if (adminEntry && adminEntry.password) realAdminPassword = adminEntry.password;
      }
    } catch (_) {}
    ...
    if (!adminExists) {
      ...
      // Always sync password from userData.json so login stays consistent
      if (adminExists.password !== realAdminPassword) {
        adminExists.password = realAdminPassword;
        await adminExists.save();
        console.log('✅ Super Admin password synced from userData.json');
      }
    }
  } catch (error) {
...
// DB connect + seed will be called after all functions are defined (see initServer below)

// Auto-upgrade existing non-admin teachers with 'free' license to 'pro_annual' (365 days)
const upgradeExistingTeachersToProAnnual = async () => {
  try {
    const oneYearFromNow = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);
    const result = await User.updateMany(
      { 
        role: { $ne: 'superadmin' }, 
        $or: [{ license: 'free' }, { license: { $exists: false } }, { license: null }] 
      },
      { 
        $set: { 
          license: 'pro_annual', 
    ...
    if (result.modifiedCount > 0) {
      console.log(`[License Migration] Upgraded ${result.modifiedCount} existing teacher(s) to Pro Annual (1 Year).`);
    }
  } catch (err) {
    console.warn('[License Migration] Notice:', err.message);
  }
};
// NOTE: upgradeExistingTeachersToProAnnual() is now called after connectDB() above
...
// ✅ Single unified startup: connect DB once, then seed + migrate all data in order
const initServer = async () => {
  try {
    await connectDB();
    await seedSuperAdmin();
    await upgradeExistingTeachersToProAnnual();
    await migrateQuizzes();
    await migrateUsers();
    console.log('✅ Server initialization complete — DB connected, seeded, and migrated.');
  } catch (err) {
...
// Repair existing quizzes that were saved without authorEmail/authorId due to old wrong schema
const repairQuizAuthorFields = async () => {
  try {
    // Fix quizzes with no authorEmail or authorId
    const broken = await Quiz.find({
      $or: [
    ...
    if (broken.length > 0) {
      for (const q of broken) {
        await Quiz.updateOne(
          { _id: q._id },
          { $set: {
            authorId: q.authorId || q.creatorId || 'official',
            authorEmail: q.authorEmail || 'official',
            authorName: q.authorName || 'AC-Kahoot! Official',
            isOfficial: q.isOfficial ?? true
          }}
        );
      }
      console.log(`[Repair] Fixed ${broken.length} quiz(zes) with missing author fields.`);
    }
  } catch (err) {
    console.warn('[Repair] Quiz author field repair error:', err.message);
  }
};
repairQuizAuthorFields();
...
app.post('/api/auth/register', async (req, res) => {
  ...
  const pricingConfig = loadBotPricing();
  const defaultTier = pricingConfig.defaultSignupTier || 'pro_annual';

  let userLicense = defaultTier;
  let licenseExpiryDate = null;
  if (userLicense === 'pro_annual') {
    licenseExpiryDate = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);
  } else if (userLicense === 'pro_monthly') {
    licenseExpiryDate = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
  }

  if (licenseKey && licenseKey.trim()) {
     if (licenseKey === 'PRO-WEB' || licenseKey.toUpperCase().startsWith('ACK-PRO')) {
       userLicense = 'pro_annual';
       licenseExpiryDate = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);
     }
  }
...
app.post('/api/auth/login', async (req, res) => {
  ...
  // Auto-upgrade existing teachers if defaultSignupTier is pro_annual and user is still free
  const pricingConfig = loadBotPricing();
  if ((pricingConfig.defaultSignupTier || 'pro_annual') === 'pro_annual' && user.license === 'free' && (!user.role || user.role !== 'superadmin')) {
    user.license = 'pro_annual';
...
app.get('/api/admin/users', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to load users from database', error: err.message });
  }
});
...
app.post('/api/quizzes', async (req, res) => {
  ...
  if ((!userId || !userEmail) && req.headers.authorization?.startsWith('Bearer ')) {
    try {
  ...
  if (!existingQuiz && newQuiz.authorEmail && newQuiz.authorEmail !== 'official') {
    ...
    if (!isPro) {
      const emailToCheck = user?.email || newQuiz.authorEmail;
      const userQuizzesCount = emailToCheck 
        ? await Quiz.countDocuments({ authorEmail: new RegExp(`^${emailToCheck}$`, 'i') }) 
        : 0;
  ...
  try {
    newQuiz.id = newQuiz.id || `quiz_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
    newQuiz.authorId = newQuiz.authorId || user?.id || 'official';
    newQuiz.authorEmail = newQuiz.authorEmail || user?.email || 'official';
    newQuiz.authorName = newQuiz.authorName || user?.name || 'AC-Kahoot! Official';

    if (existingQuiz) {
      await Quiz.updateOne({ id: newQuiz.id }, { $set: newQuiz });
    } else {
    ...
    res.status(500).json({ success: false, message: 'បរាជ័យក្នុងការរក្សាទុកវិញ្ញាសា៖ ' + err.message });
  }
});
  ...
  try {
    ...
    if (mimetype === 'application/pdf') {
      ...
      try {
        ...
        console.warn('[extract-lesson] pdf-parse extraction error:', pdfErr.message);
      }
      if (!extractedText && !apiKey) {
        throw new Error('PDF មិនមានអត្ថបទស្រង់បាន ឬជាប្រភេទរូបភាព (Image-only Scan)។');
      }
    }
...
app.post('/api/generate-quiz', async (req, res) => {
  ...
  const FREE_SERVER_AI_QUOTA = 25;
  let remainingTrialCount = FREE_SERVER_AI_QUOTA;

  const defaultSystemKey = currentBotPricing.geminiApiKey || process.env.GEMINI_API_KEY || DEFAULT_TRIAL_GEMINI_KEY;

  if (!activeApiKey) {
    // Option 1 (Hybrid Model): 25 free AI generations from server key, then require personal free key for unlimited
    const currentUsage = user ? (user.aiGenerationsCount || 0) : 0;
    if (currentUsage >= FREE_SERVER_AI_QUOTA) {
      return res.status(403).json({
        success: false,
        quotaExceeded: true,
        message: '🎉 លោកគ្រូ/អ្នកគ្រូ បានប្រើប្រាស់កូតា AI ឥតគិតថ្លៃគ្រប់ ២៥ លើកហើយ! ដើម្បីបន្តបង្កើតសំណួរ AI ដោយមិនកំណត់ (Unlimited) ពេញមួយជីវិត សូមភ្ជាប់ Google Gemini API Key ឥតគិតថ្លៃផ្ទាល់ខ្លួនរបស់អ្នក (ចំណាយពេលត្រឹម ៣០ វិនាទី)។'
      });
    }
    activeApiKey = defaultSystemKey;
    isTrialUsage = true;
    remainingTrialCount = Math.max(0, FREE_SERVER_AI_QUOTA - (currentUsage + 1));
  } else {
    // Teacher provided their own Gemini API key: Unlimited usage!
    isTrialUsage = false;
  }
          ...
          if (response.ok) {
            ...
            if (data.candidates && data.candidates[0]?.content?.parts?.[0]?.text) {
              ...
              return res.json({
                ...
                remainingTrialCount: isTrialUsage ? remainingTrialCount : 'unlimited',
                isUnlimited: !isTrialUsage
              });
            }
          } else {
...
io.on('connection', (socket) => {
  ...
  socket.on('host:create-room', ({ quiz, level, hostLicense, clientOrigin }) => {
    ...
    const hostJoinUrl = (clientOrigin && !clientOrigin.includes('localhost') && !clientOrigin.includes('127.0.0.1'))
      ? `${clientOrigin}/?pin=${pin}`
      : `http://${getLocalIpAddress()}:${process.env.PORT || 3333}/?pin=${pin}`;
    ...
    socket.emit('host:room-created', {
      ...
      joinUrl: hostJoinUrl,
      publicJoinUrl: publicUrl ? `${publicUrl}/?pin=${pin}` : (clientOrigin && !clientOrigin.includes('localhost') && !clientOrigin.includes('127.0.0.1') ? `${clientOrigin}/?pin=${pin}` : null)
    });
    console.log(`[Host] Room created: ${pin} (${quiz.title}) | URL: ${hostJoinUrl}`);
  });
...
app.post('/api/admin/bot-pricing', (req, res) => {
  ...
  const updated = {
    ...
    defaultSignupTier: (defaultSignupTier || currentBotPricing.defaultSignupTier || 'pro_annual').trim(),
