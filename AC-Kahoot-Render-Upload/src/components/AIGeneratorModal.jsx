...
export default function AIGeneratorModal({ isOpen, onClose, onSaveGeneratedQuiz, onAppendQuestions, initialLevel, currentUser, lang = 'km' }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (!topic) setTopic(`វិញ្ញាសាដកស្រង់ពី៖ ${file.name}`);
      } else {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.success && data.questions) {
        ...
        setQuizTitle(`វិញ្ញាសា AI៖ ${topic.slice(0, 35)}`);
  ...
  const handleSaveToLibrary = () => {
    ...
    const newQuiz = {
      id: `quiz-ai-${Date.now()}`,
      title: quizTitle || `វិញ្ញាសា AI៖ ${topic.slice(0, 30)}`,
      description: `វិញ្ញាសាបង្កើតដោយ Gemini AI លើប្រធានបទ "${topic}"`,
      level: level,
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md font-khmer">
        ...
        <div className="p-5 sm:p-6 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500 to-orange-600 flex items-center justify-center text-white shadow-lg">
              <Sparkles className="w-5 h-5 text-yellow-200 animate-pulse" />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
                <span>{generatedQuestions ? 'លទ្ធផលសំណួរដែលបង្កើតដោយ AI' : 'Gemini AI Quiz Generator'}</span>
                <span className="text-[10px] bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded-full border border-amber-400/30">Google Gemini</span>
              </h2>
              <p className="text-xs text-purple-200/70">
                {generatedQuestions 
                  ? `បានបង្កើត ${generatedQuestions.length} សំណួររួចរាល់ • សូមពិនិត្យ ឬចុចរក្សាទុក` 
                  : 'បង្កើតសំណួរ ជម្រើស ៤ និងការពន្យល់ស្វ័យប្រវត្តជាភាសាខ្មែរ'}
              </p>
            </div>
          </div>
            ...
            <>
              ...
              <div>
                <label className="block text-xs font-bold text-purple-200 mb-1.5 flex items-center justify-between">
                  <span>ប្រធានបទ ឬអត្ថបទសង្ខេបមេរៀន (Topic / Lesson Outline)</span>
                  <div className="flex items-center gap-2">
                    <span className="text-purple-300/60 font-normal">ភាសាខ្មែរ ឬ អង់គ្លេស</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-purple-200 mb-1.5">កម្រិតសិក្សា (Level)</label>
                  <select
                    value={level}
                  ...
                  >
                    <option value="university">🏛️ ឧត្តមសិក្សា</option>
                ...
                <div>
                  <label className="block text-xs font-bold text-purple-200 mb-1.5">ចំនួនសំណួរ (Count)</label>
                  <select
                    value={count}
                    onChange={(e) => setCount(e.target.value)}
                    className="w-full bg-black/40 border border-white/15 rounded-xl px-3 py-2 text-xs font-semibold text-white focus:outline-none focus:border-amber-400 font-khmer"
                  >
                    <option value="3">៣ សំណួរ (រហ័ស - Quick Test)</option>
                    <option value="5">៥ សំណួរ (ស្តង់ដារ)</option>
                    <option value="10">១០ សំណួរ (មធ្យម)</option>
                    <option value="15">១៥ សំណួរ</option>
                    <option value="20">២០ សំណួរ (ប្រឡងឆមាស)</option>
                    <option value="25">២៥ សំណួរ</option>
                    <option value="30">៣០ សំណួរ (វិញ្ញាសាធំ)</option>
                    <option value="40">៤០ សំណួរ</option>
                    <option value="50">៥០ សំណួរ (ប្រឡងថ្នាក់ជាតិ/បាក់ឌុប)</option>
                  </select>
                </div>
              ...
              {/* Estimated Time per Question & Total Time Badge */}
              <div className="bg-purple-950/40 border border-purple-400/30 p-3.5 rounded-2xl space-y-2.5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <label className="text-xs font-bold text-yellow-300 flex items-center gap-1.5">
                    <Clock className="w-4 h-4 text-yellow-400 animate-pulse" />
                    <span>កំណត់ម៉ោងក្នុងមួយសំណួរ (Estimated Time per Question):</span>
                  </label>
                  <span className="px-2.5 py-0.5 rounded-full bg-yellow-400/20 text-yellow-300 text-[11px] font-extrabold border border-yellow-400/30">
                    ⏱️ ម៉ោងលេងសរុប៖ ~{Math.ceil((parseInt(count, 10) * timeLimitPerQuestion) / 60)} នាទី ({count} សំណួរ)
                  </span>
                </div>

                <div className="grid grid-cols-5 gap-1.5">
                  {[
                    { val: 10, label: '10 វិនាទី' },
                    { val: 20, label: '20 វិ. (ស្តង់ដារ)' },
                    { val: 30, label: '30 វិនាទី' },
                    { val: 60, label: '1 នាទី' },
                    { val: 120, label: '2 នាទី (គិតលេខ)' },
                  ].map((preset) => {
                    const isSelected = timeLimitPerQuestion === preset.val;
                    return (
                      <button
                        key={preset.val}
                        type="button"
                        onClick={() => { sound.playClick(); setTimeLimitPerQuestion(preset.val); }}
                        className={`py-1.5 px-1 rounded-xl text-xs font-bold font-khmer transition-all border text-center ${
                          isSelected
                            ? 'bg-gradient-to-r from-yellow-500 to-amber-500 text-black border-yellow-300 shadow-md font-black scale-105'
                            : 'bg-black/50 hover:bg-white/10 text-gray-200 border-white/15'
                        }`}
                      >
              ...
              <div className="p-4 rounded-2xl bg-black/40 border border-purple-500/30 space-y-3">
                ...
                {/* Always-visible input box & Test Button */}
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  ...
                  {apiKey && (
                    ...
                    >
                      {testingKey ? <Loader2 className="w-3.5 h-3.5 animate-spin text-yellow-300" /> : <Sparkles className="w-3.5 h-3.5 text-yellow-300" />}
                      <span>{testingKey ? 'កំពុងតេស្ត...' : 'តេស្ត Key'}</span>
                    </button>
                  )}
                </div>
