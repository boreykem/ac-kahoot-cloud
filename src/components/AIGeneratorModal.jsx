...
export default function AIGeneratorModal({ isOpen, onClose, onSaveGeneratedQuiz, onAppendQuestions, initialLevel, currentUser, lang = 'km' }) {
  ...
  const handleTestKey = async () => {
    ...
    try {
      ...
      if (data.success) {
        setKeyTestStatus({
          success: true,
          message: data.message || '✅ API Key ត្រឹមត្រូវ និងដំណើរការល្អ!'
        });
        sound.playCorrect();
        ...
        setKeyTestStatus({
          success: false,
          message: data.message || '❌ Google AI Studio បដិសេធ៖ API Key មិនដំណើរការ'
        });
        sound.playIncorrect();
  ...
  const handleSaveToLibrary = () => {
    ...
    // Show success flash then close modal and return to Library
    setSavedSuccess(true);
    setTimeout(() => {
      resetForNextBatch();
      if (onClose) onClose();
  ...
  return (
    <>
      ...
      {/* ======================================================== */}
      {loading && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/90 font-khmer">
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 font-khmer">
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1">
          ...
          {!generatedQuestions ? (
            <>
              {/* Topic / Content Input */}
              <div>
                <div className="block text-xs font-bold text-purple-200 mb-1.5 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {apiKey && !isEditingKey ? (
                      <span className="text-[10px] bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded-full border border-emerald-500/40 font-bold flex items-center gap-1">
                        ...
                        <span>👑 Key ផ្ទាល់ខ្លួន (Unlimited AI)</span>
                      </span>
                    ) : (
                      <span className="text-[10px] bg-amber-500/20 text-yellow-300 px-2.5 py-0.5 rounded-full border border-yellow-500/40 font-bold flex items-center gap-1">
                        <span>✨ កូតា AI ឥតគិតថ្លៃ៖</span>
                        <span className={`font-black ${aiUsageCount >= 25 ? 'text-red-400' : 'text-yellow-300'}`}>
                          {Math.max(0, 25 - aiUsageCount)}/25 លើក
                        </span>
                      </span>
                    )}
                  </div>
                        ...
                        >
                          {testingKey ? (
                            <>
                              ...
                              <span>{lang === 'km' ? 'កំពុងតេស្ត...' : 'Testing...'}</span>
                            </>
                          ) : (
                            <>
                ...
                <p className="text-[11px] text-purple-200/70">
                  {apiKey ? (
                    <span className="text-emerald-300 font-bold">
                      ✨ API Key ផ្ទាល់ខ្លួនត្រូវបានចងចាំក្នុង Browser របស់អ្នក សម្រាប់បង្កើតសំណួររហ័សគ្មានដែនកំណត់ (Unlimited AI)។
                    </span>
                  ) : (
                    <span className="text-yellow-300/90 font-medium">
                      💡 <strong>មិនទាន់មាន API Key មែនទេ?</strong> ទុកប្រអប់ខាងលើឱ្យនៅទទេ ដើម្បីប្រើកូតា AI ឥតគិតថ្លៃ ២៥ លើកពី Server។ ភ្ជាប់ Free Key ផ្ទាល់ខ្លួនពី Google ដើម្បីបង្កើតមិនកំណត់!
                    </span>
                  )}
                </p>

                {!apiKey && aiUsageCount >= 25 && (
                  <div className="p-3 rounded-xl bg-red-500/20 border border-red-400/40 text-red-200 text-xs font-bold flex items-start gap-2">
                    <span>⚠️</span>
                    <span>លោកគ្រូ/អ្នកគ្រូ បានប្រើប្រាស់កូតា AI ឥតគិតថ្លៃគ្រប់ ២៥ លើកហើយ! សូមចុចយក និងបញ្ចូល Gemini API Key ឥតគិតថ្លៃផ្ទាល់ខ្លួនខាងលើ ដើម្បីបន្តបង្កើតសំណួរដោយ AI មិនកំណត់ (Unlimited)។</span>
                  </div>
                )}
              </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>កំពុង Upload & អានឯកសារ... {uploadProgress !== null ? `${uploadProgress}%` : '⏳'}</span>
                  </>
                ) : (
                  <>
                    <Sparkles className="w-5 h-5 text-yellow-200 animate-pulse" />
