...
export default function MasterAdminModal({ isOpen, onClose, currentUser, lang = 'km' }) {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      if (usersData.success) {
        ...
        const errMsg = usersData.message || 'Unknown error from server';
        console.warn('Users fetch failed:', usersData);
      ...
      setUsersError('មិនអាចភ្ជាប់ Server បានទេ — ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 font-khmer animate-scale-in">
      <div className="glass-panel border border-yellow-500/40 rounded-3xl max-w-5xl w-full max-h-[92vh] flex flex-col justify-between overflow-hidden shadow-2xl bg-[#17032d]">
        ...
        {/* Navigation Tabs */}
        <div className="flex items-center gap-2 sm:gap-3 px-4 sm:px-6 pt-3 border-b border-white/10 text-xs font-bold overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden bg-[#17032d] shrink-0">
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          {activeTab === 'teachers' ? (
            <>
              ...
              {/* Default Registration Plan Policy Control Card */}
              <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-900/60 via-indigo-950/70 to-purple-950/60 border border-purple-400/40 shadow-lg flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-yellow-400 animate-pulse" />
                    <h4 className="text-xs sm:text-sm font-bold text-white">
                      {lang === 'km' ? 'គោលការណ៍គណនីចុះឈ្មោះថ្មី (Default Signup Plan)' : 'Default Registration Plan'}
                    </h4>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black border ${
                      botPricing.defaultSignupTier === 'free'
                        ? 'bg-gray-500/20 text-gray-300 border-gray-400/30'
                        : 'bg-emerald-500/20 text-emerald-300 border-emerald-400/30'
                    }`}>
                      {botPricing.defaultSignupTier === 'free' ? 'Free Tier' : '⭐ Pro Annual (365 ថ្ងៃ)'}
                    </span>
                  </div>
                  <p className="text-[11px] text-purple-200/80 leading-relaxed">
                    {lang === 'km' 
                      ? 'កំណត់ប្រភេទ License ដែលគ្រូទទួលបានដោយស្វ័យប្រវត្តិនៅពេលគាត់ចុះឈ្មោះថ្មី។ លោកគ្រូអាចប្តូរមក Free វិញបានគ្រប់ពេល។'
                      : 'Set the automatic license tier for newly registered teachers. You can switch back to Free Tier anytime.'}
                  </p>
                </div>

                <div className="flex items-center gap-2 shrink-0">
                  <select
                    value={botPricing.defaultSignupTier || 'pro_annual'}
                    onChange={async (e) => {
                      const newTier = e.target.value;
                      const updated = { ...botPricing, defaultSignupTier: newTier };
                      setBotPricing(updated);
                      try {
                        await fetch('/api/admin/bot-pricing', {
                          method: 'POST',
                          headers: getAuthHeaders(),
                          body: JSON.stringify(updated)
                        });
                        sound.playCorrect();
                        setActionNotice(lang === 'km' ? `បានប្តូរគោលការណ៍ចុះឈ្មោះទៅជា៖ ${newTier === 'pro_annual' ? 'Pro 1 ឆ្នាំ (365 ថ្ងៃ)' : 'Free Tier'} រួចរាល់!` : `Default tier updated to ${newTier}`);
                        setTimeout(() => setActionNotice(''), 3500);
                      } catch (err) {
                        alert('Error updating setting');
                      }
                    }}
                    className="bg-black/60 border border-yellow-400/40 text-yellow-300 rounded-xl px-3 py-2 text-xs font-bold font-khmer focus:outline-none focus:border-yellow-300 cursor-pointer"
                  >
                    <option value="pro_annual">🟢 Pro Annual 1 ឆ្នាំ (365 ថ្ងៃ)</option>
                    <option value="free">⚪ Free Tier (កំណត់ត្រឹម 5 នាក់)</option>
                  </select>
                </div>
              </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-white/10">
                        {loading ? (
                          <tr>
                            <td colSpan={4} className="p-8 text-center text-purple-300/70">
                          ...
                          <tr>
                            <td colSpan={4} className="p-6 text-center">
                              <div className="flex flex-col items-center gap-2 text-red-400">
                                ...
                                <p className="font-bold text-sm">មានបញ្ហាក្នុងការផ្ទុកគណនីអតិថិជន</p>
                                <p className="text-xs text-red-300/70 font-mono bg-red-950/40 px-3 py-1.5 rounded-lg border border-red-500/20 max-w-md">{usersError}</p>
                                <button
                                  onClick={fetchAdminData}
                                  className="mt-2 px-4 py-1.5 rounded-lg bg-yellow-500/20 hover:bg-yellow-500/40 text-yellow-300 text-xs font-bold border border-yellow-400/30 flex items-center gap-1.5 transition-all"
                                >
                                  <RefreshCw className="w-3 h-3" />
                                  ព្យាយាមម្ដងទៀត (Retry)
                                </button>
                              </div>
                            </td>
                          </tr>
                          ...
                          <tr>
                            <td colSpan={4} className="p-8 text-center text-purple-300/50">
                              <p className="text-sm">{searchTerm ? '🔍 រកមិនឃើញគ្រូបង្រៀន' : '📭 មិនទាន់មានគណនីអតិថិជននៅឡើយ'}</p>
                            </td>
                          </tr>
                        ) : filteredUsers.map((u) => {
