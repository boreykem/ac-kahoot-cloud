...
export default function App() {
  ...
  const fetchQuizzes = async (userOverride = null) => {
    try {
      const user = userOverride !== null ? userOverride : currentUser;
      const headers = {};
  ...
  // Re-fetch quizzes whenever the logged-in user changes (login, switch account, logout)
  useEffect(() => {
  ...
  }, [isAdminModalOpen, isPricingModalOpen, isProfileModalOpen, isAuthModalOpen, isAIModalOpen, currentView]);

  useEffect(() => {
    if (isAdminModalOpen || isPricingModalOpen || isProfileModalOpen || isAuthModalOpen || isAIModalOpen || currentView !== 'home') {
      window.history.pushState({ appState: true }, '', window.location.href);
    }
  }, [isAdminModalOpen, isPricingModalOpen, isProfileModalOpen, isAuthModalOpen, isAIModalOpen, currentView]);
